    ...
    methods
        ...
        function ret = getTrait(obj, name)
            ...
            ret = reshape([obj.mesh.HE_traits(obj.index).(name)], ...
                size(obj.mesh.HE_traits(obj.index(1)).(name),2), [])';
        end
        
        function setTrait(obj, name, val)
            % Sets a trait of the referenced halfedge (halfedges). If a list
            % of halfedges is referenced, val is a cell array that contains
            % the trait of the i-th referenced halfedge in its i-th element.
            if size(val,1) > 1
                temp = num2cell(val,2);
